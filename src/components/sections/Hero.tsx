...
const awwwardsEase = [0.76, 0, 0.24, 1] as const;
        ...
        <div className="grid lg:grid-cols-2 gap-50 items-center">
          ...
          <div className="space-y-5 mt-15">
            <motion.div
              ...
              transition={{ duration: 0.8, delay: 0.4, ease: awwwardsEase }}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-zinc-300 backdrop-blur"
            >
              <div className="h-2 w-2 rounded-full bg-green-500" />
              <span className="text-[10px] text-brown-600 montserrat uppercase tracking-[0.05em]">Building BusFlow</span>
            </motion.div>
            ...
            <div>
              <span className="geist-font text-[120px] font-medium leading-[0.94] tracking-[-0.045em] text-gray-500 flex flex-col cursor-default">
                <div className="overflow-hidden">
                  <motion.span
                    initial={{ y: "110%" }} animate={isLoaded ? { y: "0%" } : {}}
                    transition={{ duration: 1.2, delay: 0.5, ease: awwwardsEase }}
                    className="block hover:text-red-400 transition-all duration-25"
                  >
                    Builder.
                  </motion.span>
                </div>
                <div className="overflow-hidden">
                  <motion.span
                    initial={{ y: "110%" }} animate={isLoaded ? { y: "0%" } : {}}
                    transition={{ duration: 1.2, delay: 0.6, ease: awwwardsEase }}
                    className="block hover:text-green-400/90 transition-all duration-250"
                  >
                    Learner.
                  </motion.span>
                </div>
                <div className="overflow-hidden">
                  <motion.span
                    initial={{ y: "110%" }} animate={isLoaded ? { y: "0%" } : {}}
                    transition={{ duration: 1.2, delay: 0.7, ease: awwwardsEase }}
                    className="block hover:text-blue-400/90 transition-all duration-250"
                  >
                    Solver.
                  </motion.span>
                </div>
              ...
              <motion.div
                ...
                className="flex items-center ml-15 mt-10 gap-8"
              >
                <a href="https://linkedin.com/in/raumayaggarwal" target="_blank" rel="noopener noreferrer" className="text-zinc-300 hover:text-[#0A66C2] transition-colors duration-300"><FaLinkedinIn size={30} /></a>
                <a href="https://github.com/imraumayaggarwal" target="_blank" rel="noopener noreferrer" className="text-zinc-300 hover:text-black transition-colors duration-300"><FaGithub size={30} /></a>
                <a href="https://x.com/yourhandle" target="_blank" rel="noopener noreferrer" className="text-zinc-300 hover:text-black transition-colors duration-300"><FaXTwitter size={30} /></a>
                <a href="mailto:youremail@gmail.com" className="text-zinc-300 hover:text-[#EA4335] transition-colors duration-300"><SiGmail size={30} /></a>
              </motion.div>
            </div>
          </div>

          {/* RIGHT: PORTRAIT WITH LINE BOIL */}
          <motion.div
            initial={{ opacity: 0, scale: 0.9 }}
            animate={isLoaded ? { opacity: 1, scale: 1 } : {}}
            transition={{ duration: 1.2, delay: 0.6, ease: awwwardsEase }}
            className="relative flex justify-center top-5"
          >
            <div className="relative">
              {/* Ambient Glow */}
              <div className="absolute inset-0 rounded-full bg-blue-200/70 blur-[120px] animate-pulse" />

              {/* 4-Frame Line Boil Filters */}
              <svg className="absolute w-0 h-0 pointer-events-none opacity-0" aria-hidden="true">
                <defs>
                  <filter id="distort-boil-1" x="-10%" y="-10%" width="120%" height="120%">
                    <feTurbulence type="fractalNoise" baseFrequency="0.048" numOctaves="3" result="noise" seed="3" />
