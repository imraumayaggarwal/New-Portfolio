...
export default function Navbar({ isRevealed = true }: { isRevealed?: boolean }) {
  const [activeSection, setActiveSection] = useState("About");
  ...
  useEffect(() => {
    const updateTime = () => {
      setTime(DayJS().tz("Asia/Kolkata").format("HH:mm"));
    };
    updateTime();
    const interval = setInterval(updateTime, 1000);
  ...
  const middleIndex = Math.floor(navItems.length / 2); // Index 2 -> "Experience"

  return (
    <motion.nav
      initial={{ y: "-100%" }}
      animate={isRevealed ? { y: "0%" } : { y: "-100%" }}
      transition={{ duration: 1.1, ease: ultraFluidEase }}
      className={`fixed left-0 w-full z-50 transition-all duration-500 ease-out ${
        scrolled ? "top-2 " : "top-0 shadow-xs"
      }`}
    >
      <Container>
        <div
          className={`h-18 flex items-center justify-between mx-auto px-8 transition-all duration-500 ease-out ${
            scrolled
              ? "w-[90%] rounded-full bg-white/20 backdrop-blur-md border border-black/30 shadow-hg"
              : "w-full text"
          }`}
        >
          {/* PROFILE BADGE (Fades in slightly as the links ripple) */}
          <motion.div
            initial={{ opacity: 0, x: -14 }}
            animate={isRevealed ? { opacity: 1, x: 0 } : { opacity: 0, x: -14 }}
            transition={{ duration: 0.8, delay: 0.4, ease: ultraFluidEase }}
          >
            <a className="text-[14px] font-medium tracking-tight flex items-center gap-2 cursor-pointer">
              <Image
                src="/image.png"
          ...
          {/* CENTER-OUT RIPPLE LINKS */}
          <div className="open-sans text-[14px] text-[#73726e] flex gap-6 tracking-tight">
            {navItems.map((item, index) => {
              // 0 for "Experience", 1 for "About" & "Projects", 2 for "Home" & "Contact"
              const distanceFromCenter = Math.abs(index - middleIndex);
              const linkDelay = 0.05 + distanceFromCenter * 0.12;
              ...
              return (
                <div key={item} className="overflow-hidden py-1">
                  ...
                  >
                    <a
                      href={`#${item}`}
                      className={`relative pb-1 transition-all duration-300 block ${
                        activeSection === item
                          ? "text-black"
                          : "text-[#73726e] hover:text-black"
                      }`}
                    >
                      {item}
                      <span
                        className={`absolute left-0 bottom-0 h-[1px] bg-blue-400/40 transition-all duration-300 ${
          ...
          {/* LOCATION / TIME (Fades in on the right edge) */}
          <motion.div
            initial={{ opacity: 0, x: 14 }}
            animate={isRevealed ? { opacity: 1, x: 0 } : { opacity: 0, x: 14 }}
            transition={{ duration: 0.8, delay: 0.4, ease: ultraFluidEase }}
            className="montserrat font-bold flex gap-2 text-gray-500 text-[10px] tracking-[0.15rem] items-center cursor-default"
          >
            <div className="uppercase">Dehradun, India</div>
            <div className="text-lg">·</div>
            <div>{time} IST</div>
